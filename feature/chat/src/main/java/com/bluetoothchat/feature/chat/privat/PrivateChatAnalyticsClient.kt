...
@Singleton
class PrivateChatAnalyticsClient @Inject constructor(private val analyticsClient: AnalyticsClient) {

    suspend fun reportScreenShown(source: String, messagesCount: Int) {
        val event = AnalyticsEvent(
            name = EVENT_SCREEN_SHOWN,
            params = mapOf(PROPERTY_MESSAGES_COUNT to messagesCount) + mapOf(PROPERTY_SOURCE to source)
        )
    ...
    suspend fun reportBluetoothEnabled() {
        val event = createBluetoothEnabledEvent(source = SOURCE_PRIVATE_CHAT)
        analyticsClient.logEvent(event)
    }

    suspend fun onBluetoothPermissionGranted() {
        val event = createBluetoothPermissionGrantedEvent(source = SOURCE_PRIVATE_CHAT)
    ...
    suspend fun onBluetoothPermissionDenied() {
        val event = createBluetoothPermissionDeniedEvent(source = SOURCE_PRIVATE_CHAT)
    ...
    suspend fun reportConnectErrorDialogShown() {
        val event = createConnectErrorDialogShownEvent(source = SOURCE_PRIVATE_CHAT)
        analyticsClient.logEvent(event)
    }

    companion object {
        private const val EVENT_SCREEN_SHOWN = "private_chat_screen_shown"
